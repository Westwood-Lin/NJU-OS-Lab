...
int main(void) {
	...
	* fs.read(buff,510)是因为在windows的VS2019上尝试读取时起始扇区存在对齐问题，读入510个字节不会影响后续
	*	注意！ linux上这里就得改为fs.read(buff,512)
	...
	* buff += … 在遍历以后，通过修改指针丢弃根目录区
	*/
	fs.read(buff, 510);//TODO! linux虚拟机上这里是 fs.read(buff, 512)! 可能跟对齐问题或者其他平台差异有关
	boot.set(buff,510);
